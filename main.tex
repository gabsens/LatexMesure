...
Solution: Soit $\epsilon' >0$. D'après 4.2, on dispose d'un ouvert $G$ tel que $A\subset G$ et $m(G\setminus A)\leq \epsilon'$. Pour tout intervalle $I$ on a \newline \centerline{$\begin{aligned}[t] m(A\cap I) + m((G\setminus A)\cap I) &\leq (1-\epsilon)m(I) + m((G\setminus A)\cap I)\\\implies m(G \cap I) &\leq (1-\epsilon)m(I) + m((G\setminus A))\\ \implies m(G \cap I) &\leq (1-\epsilon)m(I) + \epsilon' \end{aligned}$ }
