...
\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $f_n:(X,\mathcal{A},\mu)\to (\overline{\mathbb R}, \mathcal{B}(\overline{\mathbb R}))$ mesurables positives qui convergent simplement vers $f$, avec $\int f d\mu = \lim_n \int f_n d\mu <\infty$.\newline
Pour tout $E\in \mathcal A$, montrer que $\int_E f d\mu = \lim_n \int_E f_n d\mu$.
}}\\ \\
...
\fbox{
\parbox{0.9\textwidth}{
Soient $(a_n), (b_n)$ des suites de réels avec $(b_n)$ majorée. Alors\newline  \centerline{ $\liminf (a_n+b_n)\leq \liminf a_n + \limsup b_n$}
}}
\newline \newline
...
\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $D$ une partie dénombrable de $\mathbb R$, $\nu$ la mesure de comptage sur $D$ et $f:(\mathbb R, \mathcal B(\mathbb R))\to (\mathbb R, \mathcal B(\mathbb R))$ positive. Montrer que $\int f d\nu = \sum_{t\in D} f(t)$
}}\\ \\

Solution: Pour $A\in \mathcal B(\mathbb R)$ et $f=1_A$, on a $\int f d\nu = \nu(A) = \sum_{t\in D} 1_A(t)$. \newline
Pour $f=\sum_{i=1}^p a_i \chi_{A_i}$ simple positive, on a par linéarité $$\begin{aligned}
\int f d\nu &= \sum_{i=1}^p a_i \int 1_{\chi_i} d\nu \\
&= \sum_{i=1}^p a_i \sum_{t\in D} 1_{\chi_i}(t)\\
&= \sum_{t\in D}\sum_{i=1}^p a_i 1_{\chi_i}(t) \\
&= \sum_{t\in D} f(t)
\end{aligned}$$
...
\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $f\in L(X,\mathcal A,\mu)$ et $a>0$. Montrer que $\{|f|\geq a\}$ est de mesure finie. Montrer que $\{f\neq 0\}$ est $\sigma$-fini.
}}\\ \\
...
\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $f:(X,\mathcal{A},\mu)\to (\mathbb R, \mathcal{B}(\mathbb R))$ mesurable et nulle presque-partout.\newline
Montrer que $f\in L(X,\mathcal A,\mu)$ et $\int f d\mu = 0$.
}}\\ \\
...
\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $f\in L(X,\mathcal A,\mu)$ et $g:(X,\mathcal{A},\mu)\to (\mathbb R, \mathcal{B}(\mathbb R))$ mesurable tel que $f=g$ presque-partout. Montrer que $g\in L(X,\mathcal A,\mu)$ et $\int g d\mu = \int f d\mu$.
}}\\ \\
...
\subsection{} \fbox{
\parbox{0.9\textwidth}{
On considère l'espace mesurable $(\mathbb N, \mathcal P(\mathbb N))$ muni la mesure de comptage $\mu$. Montrer que $f$ est dans $L(\mathbb N, \mathcal P(\mathbb N),\mu)$ si et seulement si $\sum_n f(n)$ est absolument convergente. Montrer que dans ce cas $\int f d\mu = \sum_{n=1}^\infty f(n)$.
}}\\ \\

Solution: Dans le cadre des fonctions mesurables positives, $\int |f| d\mu = \sum_{n=1}^\infty |f(n)|$, ce qui prouve l'équivalence recherchée.\newline
D'autre part $\begin{aligned}[t] \int f d\mu &= \int f^+ d\mu - \int f^- d\mu \\
&= \sum_{n=1}^\infty f^+(n) - \sum_{n=1}^\infty f^-(n) \\
&= \sum_{n=1}^\infty \left( f^+(n) - f^-(n) \right) \\
&= \sum_{n=1}^\infty f(n)
\end{aligned}$

\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $(f_n)$ une suite de $L(X,\mathcal A,\mu)$ qui converge uniformément vers une fonction $f$, avec $\mu(X)<\infty$. Montrer que $\int f d\mu = \lim_n \int f_n d\mu$
}}\\ \\

Solution: Soit $\epsilon >0$. On dispose de $N\geq 1$ tel que $$n\geq N \implies \forall x, |f(x)|\leq \frac{\epsilon}{\mu(X)} + |f_n(x)|$$ Donc $\int |f| d\mu \leq \epsilon +  \int |f_N| d\mu < \infty$ et $f$ est intégrable.\newline
En écrivant $$f_n(x) - \frac{\epsilon}{\mu(X)} \leq f(x)\leq f_n(x) + \frac{\epsilon}{\mu(X)} $$ on obtient $$ \left| \int f d\mu - \int f_n d\mu \right|\leq \epsilon$$
Donc $\int f_n d\mu$ converge vers $\int f d\mu$. 

\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $(f_n)$ une suite de $L(X,\mathcal A,\mu)$ telle que $\sum_{n=1}^\infty \int |f_n| d\mu < \infty$. Montrer que la série $\sum_n f_n(x)$ converge presque-partout vers une fonction $f$ de $L(X,\mathcal A,\mu)$. 
}}\\ \\
...
\subsection{} \fbox{
\parbox{0.9\textwidth}{
Soit $(f_n)$ une suite de $L(X,\mathcal A,\mu)$ qui converge simplement vers une fonction $f$. Montrer que si $\lim_n \int |f-f_n| d\mu = 0$, alors $\int |f_n|d\mu \to \int |f| d\mu$.
}}\\ \\
