...
$\bullet$ La preuve précédente montre qu'on peut en plus supposer \underline{$I$ ouvert des deux côtés et borné}.
...
Solution: Supposons par l'absurde que $0$ n'est pas dans l'intérieur de $A-A$. Alors on dispose de $(x_n) \in (A-A)^{\mathbb N}$ telle que $x_n\to 0$.\newline
Comme $m(E)>0$, d'après 4.10, on dispose, pour $0<\epsilon<1$, d'un intervalle ouvert borné $I$ tel que $m(I\cap A) > (1-\epsilon)m(I)$.\newline
...
Ceci est absurde dès lors que $\epsilon <\frac 13$.
